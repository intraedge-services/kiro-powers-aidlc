...
1. Detect if AIDLC is already configured (and offer repair/re-run options)
2. Walk you through interactive configuration (project identity, team, tech stack, powers)
3. Generate a fully populated `project-config.md`
4. Create the complete `.kiro/` and `aidlc-docs/` folder structure
5. Install steering files, workflow rule-details, and hooks
...
Review `.kiro/steering/project-config.md` (generated by the setup utility) and adjust if needed.
